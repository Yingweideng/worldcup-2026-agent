...
### 3.1 match_list 板块（今日赛果）
- **数据优先级**：`worldcup_matches` 有数据时，**必须**以官方数据为准，不得凭空捏造比分。
- 每场比赛须包含：主队、主队国旗(home_flag)、客队、客队国旗(away_flag)、比分、小组/阶段、场地（venue）、美东开球时间（kickoff_et）。
- 从 `news_items` 中补充：战术亮点、关键转折、争议判罚、球员表现等叙述性内容。
- `worldcup_matches` 为空时，省略此板块。

### 3.2 schedule 板块（明日赛程）
- 数据来源：`tomorrow_matches`。
- 每条须包含：主队、主队国旗(home_flag)、客队、客队国旗(away_flag)、美东开球时间（kickoff_et）、小组/阶段。
- `tomorrow_matches` 为空时，省略此板块。

### 3.3 ranking 板块（射手榜）
- 数据来源：`top_scorers`，严格按照 `goals` 降序，最多展示 5 名。
- 每条须包含：排名、球员姓名（player_name）、所属国家队（team_name）、进球数（goals）、点球数（penalties）。
